...
from dataset.abs_dataset_cov_cls import AbsDataset
from metrics.evaluate import evaluate_classification
from models.ssraai_kmer_embedding_cls import DeepAAIKmerEmbeddingCls
import numpy as np
import random
import torch
import math
...
class Trainer(object):
    ...
    def iteration(self, epoch, pair_idx, antibody_graph_node_idx, virus_graph_node_idx, is_training=True, shuffle=True):
        ...
        antibody_graph_map_arr = get_map_index_for_sub_arr(
            antibody_graph_node_idx, np.arange(0, 3051))
        virus_graph_map_arr = get_map_index_for_sub_arr(
            virus_graph_node_idx, np.arange(0, 30))
    ...
    def train(self, display=True):
        for epoch in range(1, self.param_dict['epoch_num'] + 1):
            ...
            unseen_test_pred, unseen_test_label, unseen_test_loss = self.iteration(epoch,
                                                                                   self.dataset.test_unseen_index,
                                                                                   antibody_graph_node_idx=self.dataset.known_antibody_idx,
                                                                                   virus_graph_node_idx=self.dataset.known_virus_idx,
                                                                                   is_training=False, shuffle=False)
            ...
            if seen_valid_acc > self.min_dif:
                ...
                save_model_data_dir = '/data0/yanghy/workplace/DeepAAI-main/c_3/'
                # save_complete_model_path = osp.join(current_path,save_model_data_dir ,self.trainer_info + '_complete.pkl')
            ...
            # 存储每个时期的结果到 epoch_results 列表中
            self.epoch_results.append(res_list)
            # # 训练完成后将结果保存到文件中
    ...
    def evaluate_model(self):
        ...
        model_file_path = osp.join(current_path, '..', 'save_model_param_pred',
                                   'deep_aai_k+e',
                                   'deep_aai_kmer_embedding_cls_seed={}_param.pkl'.format(self.param_dict['seed']))
...
if __name__ == '__main__':
    ...
    if args.mode == 'train':
        for seed in range(5):
            ...
            trainer.train()
            # 存储每个随机种子的结果
            save_path = f"/data0/yanghy/workplace/DeepAAI-main/doc/cov_results/cov_results_seed{seed}.txt"
            with open(save_path, "w") as f:
                f.write(
                    "Epoch\tACC_Train\tACC_Val\tACC_Test_Seen\tACC_Test_Unseen\t"
                    "P_Train\tP_Val\tP_Test_Seen\tP_Test_Unseen\t"
                    ...
                    "AUC_Train\tAUC_Val\tAUC_Test_Seen\tAUC_Test_Unseen\t"
                    "MCC_Train\tMCC_Val\tMCC_Test_Seen\tMCC_Test_Unseen\n")
                for epoch, res_list in enumerate(trainer.epoch_results, 1):
                    f.write(f"{epoch}\t")
                    f.write("\t".join(map(str, res_list)))
                    f.write("\n")
    else:
