...
#         self.virus_pssm_linear = nn.Linear(param_dict['pssm_virus_dim'], self.h_dim)
        ...
        self.cnnmodule = CNNmodule(in_channel=436, kernel_width=self.amino_ft_dim, l=self.max_antibody_len)
        self.cnnmodule2 = CNNmodule(in_channel=1288, kernel_width=self.amino_ft_dim, l=self.max_virus_len)

        # Create SelfAttentionModules
        self.self_attn_module = SelfAttentionModule(d_model=436, num_heads=4, dropout=0.5, l=self.max_antibody_len)
        self.self_attn_module2 = SelfAttentionModule(d_model=1288, num_heads=2, dropout=0.5, l=self.max_virus_len)
