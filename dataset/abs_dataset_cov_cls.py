...
class AbsDataset():
    def __init__(self, max_antibody_len=436, max_virus_len=1288,
        ...
        # unseen -> unseen_test
        raw_train_size = self.train_index.shape[0]/5
        ...
        self.train_index = raw_train_index[np.concatenate((new_train_index*5, new_train_index*5+1, new_train_index*5+2, new_train_index*5+3, new_train_index*5+4)).astype('int64')]
        self.valid_seen_index = raw_train_index[np.concatenate((seen_valid_index*5, seen_valid_index*5+1, seen_valid_index*5+2, seen_valid_index*5+3, seen_valid_index*5+4)).astype('int64')]
        self.test_seen_index = raw_train_index[np.concatenate((seen_test_index*5, seen_test_index*5+1, seen_test_index*5+2, seen_test_index*5+3, seen_test_index*5+4)).astype('int64')]
...
#         self.dataset_info['pssm_virus_dim'] = self.protein_ft_dict['virus_pssm'].shape[-1]
        ...
        self.dataset_info['max_antibody_len'] = self.protein_ft_dict['antibody_contact_map'].shape[1]
        self.dataset_info['max_virus_len'] = self.protein_ft_dict['virus_contact_map'].shape[1]
